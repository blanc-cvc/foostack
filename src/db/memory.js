...
exports.db = {
    ...
    check: {
        peer: {
            is_server_valid: (server) => { // utils socketio parse_client_ip and add verify v4 and v6 TODO
                return typeof server == 'string' ? true : false ;
            },
            is_sid_valid: (sid) => {
                return typeof sid == 'string' ? sid.length == 20 : false ;
            },
            is_port_valid: (port) => {
            ...
            is_pub_valid: async (pub) => { // TODO review
            ...
            is_uuid_valid: (uuid) => {
                return typeof uuid == 'string' ? require('uuid').validate(uuid) : false ;
            }
        }
    },
    del: {
        peer: {
            ...
            blacklisted: (ms) => {
                for (let index = 0; index < this.db.blacklist.length; index++) {
                    if ( this.db.blacklist[index].date <= Date.now() - ms ) {
                        this.db.blacklist.splice(index, 1);
                        index--;
                    }
                }
            }
    ...
    get: {
        peer: {
            is_default_peer: (server, port, default_peers = false) => {
            ...
            index_pub: (pub, array) => {
                for (let index = 0; index < array.length; index++) {
                    if (array[index].pub === pub) { return index }
                }
            },
            exist_pub: (pub, array) => {
                return array.filter(function(peer) { return peer.pub === pub }).length == 0 ? false : true;
            },
            ...
            exist_server: (server, port) => {
                ...
                if (typeof port == 'number') { port = `${port}` }
                return this.db.peers.filter((peer) => { return (server.length > peer.server.length ? server.includes(peer.server) : peer.server.includes(server)) && (peer.port == port) }).length == 0 ? false : true;
            },
            index_server: (server, port) => {
                if (typeof server != 'string') { return false ; }
                if (typeof port == 'number') { port = `${port}` }
                for (let index = 0; index < this.db.peers.length; index++) {
                    if ( (server.length > this.db.peers[index].server.length ? server.includes(this.db.peers[index].server) : this.db.peers[index].server.includes(server)) && (this.db.peers[index].port == port) ) { return index }
                }
            },
            onlines: () => {
                return this.db.peers.filter((peer) => { return peer.socket.connected && !this.db.get.peer.is_blacklisted(peer.server, peer.port) }).map((peer) => { return { server: peer.server, port: peer.port } });
            }
        }
    },
    set: {
        ...
        webpeer: (deserialized_handshake, socket_id) => { // review TODO // think about a ttl like to delete seen > 1h
