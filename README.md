...
Currently, an instance start with a default _chainhash_ computed from "__db_memory.db.default_peers" and the blockchain file is named _chainhash_.json . An acknowledgment is sent back to the emitter (but currently nothing is really performed after that).

Connected peers are discovered by asking a list of online nodes to everyone. Added a connectivity peer list.
...
- How to test:
  - edit server.js and let only { server: '127.0.0.1', port: '8001' } in __db_memory.db.peers .
  - run this instance with npm run dev => you have an instance running on port 8001 with a _chainhash_ corresponding to 4f596ba475464f88e648aa7af875857649f5d1ce7177cf8702187debbce3451e
  - open a new terminal, edit server.js and let only { server: '127.0.0.1', port: '8002' } in __db_memory.db.peers .
...
- npm run wprod:sprod => generate a single server.bundle.js which contain everything.

#### Obfuscator 
- removed, this way the full html content can be the same for everyone and verified with something like a browser plugin.
...
#### Blockchain: _wait 20 sec before trying stdin_
  - a block contain { block: _number_, chain: _hash_, data: _json_, prev: _hash-of-previous-block_ }.
...
#### Web:
- sanitizing childNodes and prototypes keeping only what is needed ; locking Window & Document prototype.
- socketio is used via web Worker if available. If not, fallback to callback (freezing the UI).
